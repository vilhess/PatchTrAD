...
class NASA(Dataset):
    def __init__(
        self, root="data", dataset="msl", filename="P-1_", mode="train", window_size=10
    ):
        super().__init__()

        datasets_possibles = ["smap", "msl"]
        assert (
            dataset in datasets_possibles
        ), f"dataset sould be in {datasets_possibles}"
        ...
        assert (
            filename in filenames_possible
        ), f"filename must be in {filenames_possible}"
...
def get_loaders(
    root_dir="data/nasa", dataset="msl", filename="M-7", window_size=10, batch_size=32
):
