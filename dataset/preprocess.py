...
def processing_smd(dataset_folder="data/smd"):
    ...
    for filename in file_list:
        if filename.endswith(".txt"):
            _ = load_and_save(
                "train", filename, filename.strip(".txt"), dataset_folder, output_folder
            )
            _ = load_and_save(
                "test", filename, filename.strip(".txt"), dataset_folder, output_folder
            )
            _ = load_and_save(
                "test_label",
                filename,
...
if __name__ == "__main__":
    ...
    smd_raw_base = (
        "https://raw.githubusercontent.com/NetManAIOps/OmniAnomaly/refs/heads/master/"
    )
    ...
    if os.path.exists("data/swat/SWaT_Dataset_Normal_v1.xlsx") and os.path.exists(
        "data/swat/SWaT_Dataset_Attack_v0.xlsx"
    ):
        print(f"Processing SWAT")
        processing_swat()
    else:
        print(
            f"SWAT dataset files not found. Please claims the files from https://itrust.sutd.edu.sg/itrust-labs_datasets/dataset_info/"
        )
