...
class _ScaledDotProduct(nn.Module):
    def __init__(self, d_model, n_heads, attn_dp=0.0):
        super().__init__()
...
class _MultiHeadAttention(nn.Module):
    def __init__(
        ...
        self.to_out = nn.Sequential(
            nn.Linear(n_heads * d_v, d_model), nn.Dropout(proj_dp)
        )
...
class TSTEncoderLayer(nn.Module):
    def __init__(
        self, d_model, n_heads, d_k=None, d_v=None, d_ff=256, attn_dp=0.0, dp=0.0
    ):
        super().__init__()
        assert (
            not d_model % n_heads
...
class TSTiEncoder(nn.Module):
    def __init__(
        ...
        self.W_pos = PositionalEncoding(
            q_len=patch_num, d_model=d_model, normalize=normalize, learn_pe=learn_pe
        )
    ...
    def forward(self, x):
        ...
        )  # bs*nvars x num_patches x d_model    (channel indep)
        x = self.dp(x + self.W_pos)
        x = self.encoder(x)
        x = torch.reshape(
            x, (-1, n_vars, x.shape[-2], x.shape[-1])
        )  # bs x nvars x num_patches x d_model
        x = x.permute(0, 1, 3, 2)
...
class PatchHead(nn.Module):
    def __init__(self, n_vars, d_model, patch_len, head_dp=0.0):
        super().__init__()
...
class PatchTrad(nn.Module):
    def __init__(self, config):
        ...
        self.patcher = Patcher(
            window_size=window_size, stride=stride, patch_len=patch_len
        )
        ...
        self.head_layer = PatchHead(
            n_vars=n_vars, d_model=d_model, patch_len=patch_len, head_dp=head_dp
        )
